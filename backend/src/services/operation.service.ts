...
import type { CapitalSplit, Modalidad, Operacion, Plan } from "../types"
import {
  calculateFinancing,
  getNextDueDate,
...
import { toDomainPlan } from "./state.service"
import { requireModalidad, requireRecord } from "./validation.service"
...
export async function createOperation(
  ...
  const [configurationRows, planRows, existingClientRows] = await db.batch([
    db
      .select()
      .from(configuracion)
  ...
  const plan: Plan = input.operacion.planCustom
    ? {
        id: `p_${crypto.randomUUID()}`,
        ...input.operacion.planCustom,
      }
    : planRows[0]
      ? toDomainPlan(planRows[0])
      : (() => {
          throw new AppError(
  ...
  const { updatedConfiguration } = await db.transaction(async (tx) => {
    ...
    await tx.insert(operaciones).values({
      ...
      fechaInicio: toIsoDate(now),
      pagosRealizados: 0,
      estado: "al-dia",
      proximoVencimiento: getNextDueDate(plan.frecuencia, now),
    })
...
function toCapitalResponse(configuration: {
  ...
  activoEfectivo: number
  activoTransferencia: number
}) {
  return {
    caja: {
