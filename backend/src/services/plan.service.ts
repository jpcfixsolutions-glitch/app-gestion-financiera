...
import { clientes, operaciones, planes } from "../models/schema"
import { parsePlanInput } from "./finance-rules.service"
import { toDomainPlan } from "./state.service"

export async function createPlan(empresaId: string, value: unknown) {
...
export async function deletePlan(empresaId: string, id: string) {
  if (!id || id.length > 100) {
    throw new AppError(
